...
def main(page: ft.Page):
    ...
    lbl_login_title = ft.Text("EApp 6.5 Cloud", size=32, weight="extrabold", color="#1E293B")
    ...
    def pedir_ruta_guardado(nombre_por_defecto, titulo):
        # 👑 MODO DIOS: Le preguntamos a Flet si estamos en la Nube
        if not page.web:
            try:
                # Solo importamos tkinter si estamos 100% seguros de estar en PC
            ...
            # En la nube devolvemos el nombre por defecto directo
            return nombre_por_defecto
    ...
    appbar_principal = ft.AppBar(
        leading=ft.Container(content=ft.Text(value="☁️", size=24), padding=10), 
        title=lbl_appbar, 
        bgcolor="#1E293B", 
        center_title=True,
        actions=[btn_lang]
